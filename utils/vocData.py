...
# 中文分词，先采用jiaba
class Seg(object):
    def __init__(self, stopword_filepath="./stopword.txt"):
        self.stopwords = FileObj(stopword_filepath).read_lines()

    def cut(self, sentence, stopword=False):
...
class DataSet():
    ...
    def saveFile(self, voc, pairs):
        ...
        with open("./vocData_{}.pkl".format(self.corpus_name), 'wb') as outp:
    ...
    def loadData(self):
        ...
        with open("./vocData_{}.pkl".format(self.corpus_name), 'rb') as inp:
            voc = pickle.load(inp)
